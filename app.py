...
def success():
    if request.method == 'POST':
        ...
        if f_name[-1] != 'wav':
            ...
            # Process the metadata values to ensure they are scalar
            mtdt_values = []
            for value in mtdt.values():
                if isinstance(value, (np.ndarray, list)):  # Check if the value is a sequence
                      mtdt_values.append(np.mean(value))  # Take the mean if it's a sequence
                else:
                     mtdt_values.append(value)  # Otherwise, append the scalar value
            mtdt_scaled = scaler.transform([np.array(mtdt_values).ravel()])
